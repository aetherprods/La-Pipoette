...
class InfoTaker extends React.Component {
    ...
    identify(name, color) {
        ...
        that.props.socket.addEventListener('message', this.confirm(), true); 
        that.props.socket.send(JSON.stringify({type:"identification", name: name, color: color}));
    ...
    connect(name, color, submitted) { //async WebSocket connection which resolves the socket upon receiving OPEN state from the server
        ...
        connection.then(socket => {//first we submit our name and color
            ...
            return (this.identify(name, color));            
        }, reason => {
            console.error(reason);
        });
        connection.then(userID => {//then we set the userID given by the server
